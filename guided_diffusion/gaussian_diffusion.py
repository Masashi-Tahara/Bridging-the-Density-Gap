...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            # print(model_output)
            nancheck = torch.isnan(model_output)
            if nancheck[0, 0,0,0].item() == True:
                print('nan')
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
           ...
           for i in indices:
                ...
                with th.no_grad():
                    if img.shape != (shape[0], 2, 256, 256):
    ...
    def ddim_sample(
        ...
        cb = x[:, -1:, ...]
        eps = self._predict_eps_from_xstart(cb, t, out["pred_xstart"])

        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, cb.shape)
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod_prev, t, cb.shape)
        sigma = (
                eta
